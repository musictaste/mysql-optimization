...
/var/lib/mysql/sakila中可以看到employees#P#p0.ibd、employees#P#p1.ibd、employees#P#p2.ibd、==employees#P#p3.ibd==

插入数据后四个文件的大小还是一样的，**不要通过文件大小来判断是否插入数据**；**可以通过文件更新时间（时间粒度为分钟）**
...
cd /var/lib/mysql/sakila  //中可以看到employees#P#p0.ibd、employees#P#p1.ibd、employees#P#p2.ibd、employees#P#p3.ibd

ll //四个文件的大小是一样的

插入数据  //四个文件的大小还是一样的，不要通过文件大小来判断是否插入数据；可以通过文件更新时间（时间粒度为分钟）

```

​		3、**可以使用less than maxvalue**来避免此种情况
...
​		5、**可以使用date类型进行分区**：若希望根据每个员工离开公司的年份进行划分，如**year(separated)**
...
​		6、==可以使用函数根据range的值来对表进行分区，如**timestamp   unix_timestamp()**==
